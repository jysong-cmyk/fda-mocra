  ...
  return (
    <header className="sticky top-0 z-50 border-b border-emerald-950/10 bg-stone-50/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:h-[4.25rem] sm:px-6">
        <Link href="/" className={`shrink-0 ${k}`}>
        ...
        <nav className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
          {page === "register" ? (
            <Link
              href="/"
              className={`rounded-full border border-emerald-950/25 bg-white px-4 py-2 text-sm font-medium text-emerald-950 shadow-sm transition hover:border-amber-400/50 hover:bg-stone-50 ${k}`}
            >
              메인
            </Link>
          ) : (
            <>
              ...
              <Link
                href="/register"
                className={`max-w-[11.5rem] rounded-full border border-amber-400/40 bg-emerald-950 px-3 py-2 text-center text-[10px] font-semibold leading-tight text-stone-50 shadow-sm transition hover:bg-emerald-900 hover:ring-2 hover:ring-amber-400/25 sm:max-w-none sm:px-5 sm:text-sm sm:leading-snug ${k}`}
              >
